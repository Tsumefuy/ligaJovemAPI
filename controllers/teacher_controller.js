...
module.exports.controller = (app) => {
    ...
    app.get('/api/teacher', serverServices.verifyJWT, async (req, res) => {
        ...
        //let currentTokenDB = await serverServices.getCurrentTokenDB(JSON.parse(token));
        console.log(token);
        //console.log(currentTokenDB[0].token);
        
        let userId = await serverServices.getUserIdByToken(token);
        //let userId = await serverServices.getUserIdByToken(JSON.parse(token));
    ...
    app.get('/api/teacher/context', serverServices.verifyJWT, async (req, res) => {
        ...
        if (userId[0]) {
            let response = await getContext(userId[0].userId);
...
async function getContext(userId) {
    ...
    db.query('SELECT name FROM persons WHERE id=?', [userId], (error, result_user) => {
        if (error) { rejeitado(error.code); return; }
        if (result_user[0]) {
            json = {
                name: result_user[0].name,
            ...
            db.query('select distinct rooms.id, rooms.course, rooms.mode from rooms inner join connection on rooms.id = connection.room_id and connection.user_id = ?', [userId], (error, result_rooms) => {
                if (error) { rejeitado(error.code); return; }
                for (let i=0; i < result_rooms.length; i++) {
                    let data = {
                        id: result_rooms[i].id,
                    ...
                    db.query('select day, init, end, loc from connection where room_id = ? and user_id = ?', [result_rooms[i].id, userId], (error, result_rmData) => {
                        if (error) { rejeitado(error.code); return; }
